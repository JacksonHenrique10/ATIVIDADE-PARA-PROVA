...
class Tarefa {
    ...
    static async listarTarefas() {
        try {
            ...
            const [rows] = await conn.query(`SELECT T.id_tarefa,T.id_usuario,T.descricao, T.equipe, T.prioridade, T.data_cadastro,T.data_cadastro,
                T.status FROM TAREFA T INNER JOIN USUARIO U ON T.id_usuario = U.id_usuario;`);
